...
def plot_map(N, k, coordinates):
    ...
    i = 0
    print(f'Numero de linhas {len(lines)}')
    print(len(coordinates))
    for row in lines:
        print('.',end=' ')
        fig.add_trace(go.Scattermap(lon=[lines[i][0][0],lines[i][1][0]], lat=[lines[i][0][1],lines[i][1][1]], mode='lines', showlegend = False, line=dict(color='#ffcb03')))
        i += 1
    ...
    # fig.show()
    plotly.offline.plot(fig, filename='gas2.html')


# Data Import Path
SENSORS_CSV   = 'dados_completo.csv'
